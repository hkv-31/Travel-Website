...
# REGEX VALIDATION USING STATIC METHOD
class Validator:
    @staticmethod
    def validate_registration(username, email, password):
        ...
        if not re.match(username_pattern, username):
            flash("Invalid username! Must be 6-16 characters.", "error")
            return False
        if not re.match(email_pattern, email):
            flash("Invalid email format!", "error")
            return False
        if not re.match(password_pattern, password):
            flash("Password must be 8-32 characters with letters and numbers.", "error")
            return False
        return True
...
def register():
    if request.method == 'POST':
        ...
        if User.query.filter_by(email=email).first():
            flash('Email already registered. Try logging in instead.', 'error')
            return redirect(url_for('register'))'''
